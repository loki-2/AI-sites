...
import { Edit2, Share2, Check, Link2, Github, Linkedin, Twitter, BadgeCheck } from "lucide-react";
import { BADGE_EMOJIS } from "@/lib/utils";
import useSWR from 'swr';

// Removed AVAILABLE_BADGES and SKILL_LEVELS constants
...
export default function ProfilePage() {
    ...
    const [isMvpBuilder, setIsMvpBuilder] = useState(false);
    ...
    // If empty, the user has not completed their profile setup
    const isProfileIncomplete = !profile?.badges || profile.badges.length === 0;
    ...
    useEffect(() => {
        if (profileSWRData) {
            ...
            if (profileSWRData.profile) {
                ...
                setIsMvpBuilder(profileSWRData.profile.is_mvp_builder || false);
            }
            setLoading(false);
    ...
    const handleSaveProfile = async () => {
        ...
        const { error } = await supabase
            .from('vibecoder_profiles')
            .update({ bio, badges, social_url: socialUrl, availability, build_tags: buildTags, is_mvp_builder: isMvpBuilder })
            .eq('id', user.id);
    ...
    // handleBadgeLevelChange removed
                            ...
                            {isProfileIncomplete ? (
                                <div className="space-y-6 bg-card border border-primary shadow-sm p-6 rounded-xl animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    ...
                                    <div className="space-y-4">
                                        <Label className="block text-sm font-bold uppercase tracking-wider text-muted-foreground">MVP Builder Status</Label>
                                        <div className="flex items-center gap-4 p-4 border border-border rounded-xl bg-background/50 hover:bg-muted/30 transition-colors">
                                            <div className="flex-grow">
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <h4 className="font-semibold text-foreground">MVP Builder</h4>
                                                </div>
                                                <p className="text-xs text-muted-foreground leading-relaxed">Let founders know you're an expert at shipping early-stage MVPs.</p>
                                            </div>
                                            <button
                                    ...
                                    <div className="space-y-4">
                                        <Label className="block text-sm font-bold uppercase tracking-wider text-muted-foreground">Expertise</Label>
