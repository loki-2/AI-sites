...
export function GithubActivityWidget({ username, isOwner, onConnected, onDisconnected }: GithubWidgetProps) {
    ...
    if (loading) {
        return (
            <div className="w-full h-40 bg-muted/20 animate-pulse rounded-sm border border-border/50 flex items-center justify-center">
    ...
    if (!githubUser && isOwner) {
        return (
            <div className="w-full p-8 bg-card border border-border/50 rounded-sm flex flex-col items-center justify-center text-center gap-4 shadow-sm">
