...
export function VibecoderCard({ profile }: { profile: VibecoderProfile }) {
    ...
    return (
        <Card
            ...
            className="cursor-pointer text-left rounded-3xl border-border/50 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden group-hover:border-primary/50 bg-card flex flex-col h-full"
        >
            <CardContent className="p-6 sm:p-8 flex-grow flex flex-col">
                ...
                <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                        {profile.avatar_url ? (
                            ...
                            <img src={profile.avatar_url} alt={profile.name} className="w-8 h-8 rounded-full object-cover ring-2 ring-border/50" />
                        ) : (
                ...
                <div className="flex items-center gap-6 sm:gap-8 mb-6">
                    ...
                    <div className="flex flex-col items-start">
                        ...
                        <span className="text-xs font-semibold text-muted-foreground tracking-wider uppercase">In Progress</span>
                    </div>
                    <div className="flex flex-col items-start">
                ...
                {projects.length > 0 && (
                    <div className={`grid gap-3 ${projects.length === 2 ? 'grid-cols-2' : 'grid-cols-1'} mt-auto`}>
                        {projects.map(project => {
                            ...
                            return (
                                <div key={project.id} className="relative aspect-video rounded-xl overflow-hidden bg-muted group/project border border-border/50">
                                    {image ? (
                                        // eslint-disable-next-line @next/next/no-img-element
                                    ...
                                    {/* Hover Overlay */}
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/project:opacity-100 transition-opacity duration-300 flex items-center justify-center p-4 text-center backdrop-blur-sm">
